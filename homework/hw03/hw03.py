...
def taxicab(a, b):
    ...
    return abs(street_a - street_b) + abs(av_a - av_b)
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        return [label(t)] + [replace_leaf(branch, old, new) for branch in branches(t)]
...
def balanced(m):
    ...
    if total_weight(left_side) * length(left(m)) == total_weight(right_side) * length(right(m)):
        return True and balanced(right_side) and balanced(left_side)
    return False
